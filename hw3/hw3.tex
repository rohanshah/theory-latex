...
Because $R \subseteq QxQ$ is finite, there must be some $i_0$ such that $R_{i_0+1} = R_{i_0}$.

\medskip

Prove that $R_{i_{0}}$ is the smallest forward closure
containing $R$.
If $\delta (p,q) \in R_{i_0}$ then $(\delta(p, a), \delta(q, a)) \in R_{i_0+1}$.
But since $R_{i_0+1} = R_{i_0}$ then $(\delta(p, a), \delta(q, a))$
...
If we can show this, it is immediate that $p \equiv q$. Prove by induction
